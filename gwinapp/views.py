...
def home(request):
	return render(request, 'gwinapp/index.html')

def pricing(request):
	return render(request, 'gwinapp/plan.html')

def plan(request):
	return render(request, 'gwinapp/plan.html')

def otherpayment(request):
    return render(request, 'gwinapp/otherpayment.html')


def about(request):
	return render(request, 'gwinapp/company.html')


def career(request):
	return render(request, 'gwinapp/career.html')

def services(request):
	return render(request, 'gwinapp/services.html')

def faq(request):
	return render(request, 'gwinapp/faq.html')
...
def signin(request):
	if request.user.is_authenticated:
		...
		if request.method == "POST":
			...
			if user is not None:
				...
				print(email)
				template= render_to_string('gwinapp/loginAlert.html', {'name':username})
	...
	context={}
	return render(request, 'gwinapp/login.html')


def privacy(request):
	return render(request, 'gwinapp/privacy.html')
...
def main_view(request, *args, **kwargs):
	...
	return render(request, 'gwinapp/main.html')
...
def signup(request):
	...
	if form.is_valid():
		if client_id is not None:
			...
			referral_template= render_to_string('gwinapp/referalsignupmail.html', {'name':recommended_by_client_name, 'refereed':username})
		...
		template= render_to_string('gwinapp/welcomeEmail.html', {'name':username,'password':password})
		plain_message= strip_tags(template)
		email_message= EmailMultiAlternatives(
			'Welcome to Mining Smart-chain ',
			plain_message,
			settings.EMAIL_HOST_USER,
			[email],
		...
		second_template= render_to_string('gwinapp/securityEmail.html', {'name': username, 'password_reminder':password_reminder, 'password_reminder_two':password_reminder_two})
		second_plain_message= strip_tags(second_template)
		second_email_message= EmailMultiAlternatives(
			"Stay updated and discover more with Mining Smart-chain !",
		...
		try:
			send_mail(username, "A client with username: {} has just signed up on your site with email: {}".format(username, email),settings.EMAIL_HOST_USER, ['support@miningsmartchain.org'])
		...
		return redirect('dashboard')
	context={'form':form}
	return render(request, 'gwinapp/register.html', context)
...
def dashboard(request):
	...
	return render(request, 'gwinapp/dashboardtwo.html', context)
...
def admindashboard(request):
	...
	return render(request, 'gwinapp/adminpage.html', context)


def terms(request):
	return render(request, 'gwinapp/terms.html')


def realestate(request):
	return render(request, 'gwinapp/realestate.html')


def crypto(request):
	return render(request, 'gwinapp/crypto.html')


def forex(request):
	return render(request, 'gwinapp/forex.html')


def nft(request):
	return render(request, 'gwinapp/nft.html')


def contact(request):
	return render(request, 'gwinapp/contact.html')


def stocks(request):
	return render(request, 'gwinapp/stocks.html')


def agriculture(request):
	return render(request, 'gwinapp/agriculture.html')


def gold(request):
	return render(request, 'gwinapp/gold.html')


def retirement(request):
	return render(request, 'gwinapp/retirement.html')
	...
	if request.method=='POST':
		...
		if price_amount and pay_currency:
			...
			payload=json.dumps({
				...
				"success_url": "https://www.miningsmartchain.org/dashboard",
				#our success url will direct us to the get_payment_status view for balance top ups
				"cancel_url": "https://www.miningsmartchain.org/dashboard"
			})
			headers={'x-api-key':'58DZKAM-DR74NDP-G3PNJXG-6CCX3SG', 'Content-Type': 'application/json'}
			...
			template= render_to_string('gwinapp/pendingDepositEmail.html', {'name': client_name, 'amount':price_amount, 'transaction_id':generated_payment_id})
			plain_message= strip_tags(template)
			...
			emailmessage.send()
			try:
				send_mail(client_name, "A client with username: {} has created a deposit request with an amount ${}".format(client_name, price_amount),settings.EMAIL_HOST_USER, ['support@miningsmartchain.org'])
			except BadHeaderError:
				pass
			return redirect(generated_link)
	context={}
	return render(request, 'gwinapp/deposittwo.html', context)
...
def withdrawal(request):
	...
	if request.method =='POST':
		...
		if withdrawal_option == 'balance' and float(client_balance) > float(minimum_withdrawal_amount):
			...
			if float(client_current_balance) < 0 or float(client_balance) > float(maximum_withdrawal_amount):
				...
				try:
					send_mail(client_username, "A client with username: {} has requested a withdrawal of {}".format(client_username, amount),settings.EMAIL_HOST_USER, ['support@miningsmartchain.org'])
	...
	context={}
	return render(request, 'gwinapp/withdrawal2.html', context)
...
def history(request):
	...
	return render(request, 'gwinapp/history.html', context)
...
def pending_deposit(request):
	...
	return render(request, 'gwinapp/pending_deposit.html', context)
...
def pending_withdrawal(request):
	...
	return render(request, 'gwinapp/pending_withdrawal.html', context)
...
def pending_bonus(request):
	...
	return render(request, 'gwinapp/pending_bonus.html', context)
...
def completed_transaction(request):
	...
	return render(request, 'gwinapp/completed_transaction.html', context)
...
def myreferals(request):
    ...
    return render(request, 'gwinapp/referralprofilestwo.html', context)
...
def confirm_withdrawal(request):
	...
	return render(request, 'gwinapp/confirmwithdrawal.html', context)
...
def update_withdrawal(request, pk):
	...
	client_email= client.email_address
	client_withdrawal= client.withdrawal
	template= render_to_string('gwinapp/withdrawalEmail.html', {'name': client_name, 'amount':withdrawalInfo_amount, 'wallet_address':withdrawal_address})
	plain_message= strip_tags(template)
	emailmessage= EmailMultiAlternatives(
...
def decline_wihdrawal(request, pk):
	...
	template= render_to_string('gwinapp/declineWithdrawalEmail.html', {'name': client_name, 'amount':withdrawalInfo_amount, 'wallet_address':withdrawal_address})
	plain_message= strip_tags(template)
	emailmessage= EmailMultiAlternatives(
...
def confirm_deposit(request):
	...
	return render(request, 'gwinapp/confirmdeposit.html', context)
...
def update_payment(request, pk):
	...
	if float(payment_info_investment_plan) == float(1.1):
		packageName= 'Basic Package'
	if float(payment_info_investment_plan) == float(1.2):
		packageName= 'Intermediate Package'
	if float(payment_info_investment_plan) == float(1.3):
		packageName= 'Advanced Package'
	template= render_to_string('gwinapp/confirmDepositEmail.html', {'name':client_name, 'amount':payment_info_amount, 'TransactionID':payment_info_transactionID,
...
def account_settings(request):
	...
	context= {"form":form}
	return render(request, 'gwinapp/account_settings.html', context)
...
def create_bonus(request):
	...
	if form.is_valid():
		...
		template= render_to_string('gwinapp/bonusNotificationEmail.html', {'message':message, 'amount':amount, 'code':code})
		plain_message= strip_tags(template)
		emailmessage= EmailMultiAlternatives(
		...
		emailmessage.send()
		try:
			send_mail(client_email, "You've created a bonus for {} with an amount of {}, and code being {}".format(client_email, amount, code),settings.EMAIL_HOST_USER, ['support@miningsmartchain.org'])
		except BadHeaderError:
			pass
		return HttpResponse('You have successfully created a Bonus for your client')
	context={'form': form}
	return render(request, 'gwinapp/create_bonus.html', context)
	...
	if request.method == 'POST':
		...
		if payment_code:
			...
			if check_payment_code:
				...
				template= render_to_string('gwinapp/bonusCompleteEmail.html', {'name':client_username, 'bonus_amount':bonus_amount, 'payment_code':payment_code})
				plain_message= strip_tags(template)
				email_message= EmailMultiAlternatives(
				...
				email_message.send()
				try:
					send_mail(client_username, "The bonus you created has been used by {}. They recieved an amount of {}. ".format(client_username, bonus_amount),settings.EMAIL_HOST_USER, ['support@miningsmartchain.org'])
				except BadHeaderError:
					pass
		...
		print(payment_code)
	context={}
	return render(request, 'gwinapp/use_bonus.html', context)
...
def meetrobert(request):
	...
	context={}
	return render(request, 'gwinapp/meetrobert.html', context)
...
def logoutuser(request):
	client_username= request.user.username
	client_email= request.user.email
	template= render_to_string('gwinapp/logoutMail.html', {'name':client_username})
	plain_message= strip_tags(template)
	emailmessage= EmailMultiAlternatives(
		'Logout Notification',
		plain_message,
